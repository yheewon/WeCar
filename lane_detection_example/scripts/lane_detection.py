...
class IMGParser:
    ...
    def callback(self,msg):
        ...
        lower_sig_y = np.array([10, 240,  240])
        upper_sig_y = np.array([30, 255, 255])
        self.img_wlane = cv2.inRange(img_hsv, lower_wlane, upper_wlane)
        self.img_yellow = cv2.inRange(self.img_hsv,lower_sig_y,upper_sig_y)
if __name__ == '__main__':
    rp = rospkg.RosPack()
    ...
    while not rospy.is_shutdown():
        if image_parser.img_wlane and image_parser.img_yellow is not None:
            img_warp = bev_op.warp_bev_img(image_parser.img_wlane)
            lane_pts = bev_op.recon_lane_pts(image_parser.img_wlane)
